...
export default function InboxPage() {
  ...
  const loadInbox = async () => {
    ...
    if (unreadIds.length > 0) {
      await supabase.from('documents').update({ is_read: true }).in('id', unreadIds)
    }
  }
  ...
  const handleDelete = async (fileId, filePath, fileName) => {
    ...
    const { error: dbError } = await supabase.from('documents').delete().eq('id', fileId)
    if (dbError) {
  ...
  const handleMove = async (fileId, category) => {
    const { error } = await supabase.from('documents').update({ category }).eq('id', fileId)
    if (error) {
    ...
    <div className="h-screen flex flex-col" style={bgStyle}>
      ...
      <div className="shrink-0 flex items-center justify-between px-4 md:px-6 py-4 border-b border-white/10 bg-white/10 backdrop-blur-lg">
        <div className="flex items-center gap-3">
          ...
          <h1 className={`text-xl md:text-2xl font-bold ${titleColorClass}`}>Inbox</h1>
        </div>
                ...
                {files.map((file) => {
                  ...
                  return (
                    <div key={file.id}>
                      ...
                      <li className="relative flex items-center justify-between gap-2 bg-white/10 rounded-xl px-4 py-3">
